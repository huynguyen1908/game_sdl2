...
bool GameMain::LoadData()
{
    ...
    m_TTime.SetColor(ColorBase::WHITE_COLOR);
    m_TTime.SetPos(SCREEN_WIDTH - 150, 15);
...
void GameMain::Loop()
{
    ...
    while (bQuit == false)
    {
        ...
        else if (m_GameState == GameStatus::GAME_PLAYING)
        {
            ...
            {
                is_end_game = true;
                if (m_Mark >= m_MarkGoal)
                {
                    std::string sData = "You Win";
                    m_EndMenu.SetTextContent(sData, 0);
                    SoundGame::GetInstance()->PlaySoundGame(SoundGame::GAME_TIMEOUT);
                }
                else
                {
                    SoundGame::GetInstance()->PlaySoundGame(SoundGame::GAME_FAIL);
...
void GameMain::RenderTimeDown(bool is_active)
{
    // Time
    if (is_active == true)
    {
        m_Frame1s++;
        if (m_Frame1s == FRAMES_PER_SECOND)
        {
            m_TimeDown--;
            m_Frame1s = 0;
        }
    ...
    std::string str_val = std::to_string(m_TimeDown);
    std::string str_time = "Time: " + str_val;
    m_TTime.SetText(str_time);
    m_TTime.LoadFromRenderedText(m_Font30, m_Screen);
